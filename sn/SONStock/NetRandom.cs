...
namespace Lidgren.Library.Network
{
	...
	public sealed class NetRandom
	{
		...
		public static NetRandom Default = new NetRandom();
		...
		public NetRandom()
		{
			...
			Reinitialize((int)Environment.TickCount + m_extraSeed);
			m_extraSeed++; // in case several initializations at the same time
		}
		...
		#region Public Methods [Reinitialization]
		...
		public float NextFloat()
		{
			...
			// to an int is very fast (the allocated bits remain the same) and so the overall effect 
			// of the extra cast is a significant performance improvement.
			return (float)(REAL_UNIT_INT * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8)))));
		}
		...
		public float NextFloat(float roof)
		{
			...
			// to an int is very fast (the allocated bits remain the same) and so the overall effect 
			// of the extra cast is a significant performance improvement.
			float f = (float)(REAL_UNIT_INT * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8)))));
		...
		public bool Chance(float percentChance)
		{
			...
			// to an int is very fast (the allocated bits remain the same) and so the overall effect 
			// of the extra cast is a significant performance improvement.
			double hit = (REAL_UNIT_INT * (int)(0x7FFFFFFF & (w = (w ^ (w >> 19)) ^ (t ^ (t >> 8)))));
			if (hit < percentChance)
				return true;
			return false;
		}
		...
		{
			if (buffer == null)
				throw new ArgumentNullException("buffer", "Invalid buffer object.");
